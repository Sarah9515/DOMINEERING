...
public class   Cellu extends javax.swing.JPanel implements MouseMotionListener, MouseListener {
            ...
            for(int i = 0; i < 8; i++)
            {
                for(int j = 0; j < 8; j++)
                {
                   ...
                   // {
                       ...
                       Cellu celluleBelow = getCelluleBelow();
                if (celluleBelow != null) {
                    celluleBelow.drawPion(Color.black);
                    celluleBelow.repaint();
                }
...
public void mouseClicked(MouseEvent e) {
    if (pion != null) {
        ...
        pion = new Pion(Color.black);
        int x = e.getComponent().getY() / TAILLE_CELL;
        int y = e.getComponent().getX() / TAILLE_CELL;

        // Check if there is a cell below the clicked cell
       /* int cellBelowX = x + 1;
        int cellBelowY = y;

        if (cellBelowX < 8) { // Ensure that there is a cell below
            // Calculate the index of the cell below
            int cellBelowIndex = cellBelowX * 8 + cellBelowY;

            // Access the cell below and color it
            Cellu celluleBelow = Outils.getSpecificCellule(cellBelowX, cellBelowY);
            if (celluleBelow != null) {
                celluleBelow.drawPion(Color.black);
                celluleBelow.repaint();
            }
        ...
        // Notify your application about the click
        // Outils.isClicked = true;
        // Outils.moveIndex = this.getId();

        // Repaint the clicked cell
        this.repaint();
    }
...
public void mouseClicked(MouseEvent e) {
    if (pion != null) {
        ...
        // Create a Pion with a black color
        pion = new Pion(Color.black);
        ...
        // Check if there is a cell below the clicked cell
        if (x < 7) { // Ensure that there is a cell below
            // Access the cell below and color it
            Cellule celluleBelow = Outils.getSpecificCellule(x + 1, y);
            if (celluleBelow != null) {
                celluleBelow.drawPion(Color.black);
                celluleBelow.repaint();
            }
